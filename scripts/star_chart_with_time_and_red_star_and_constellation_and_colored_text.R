...
constellation_lines_sf_boo <- const_all %>%
  filter(id == "Boo") %>%
  st_wrap_dateline(options = c("WRAPDATELINE=YES", "DATELINEOFFSET=180")) %>%
  st_transform(crs = queens_new_york) %>%
  st_intersection(hemisphere) %>%
  mutate(geometry = geometry * flip)
st_crs(constellation_lines_sf_boo) <- queens_new_york
...
library(ggplot2)
library(grid)
# Optionally install ggtext if you want colored caption
...
p <- ggplot() +
  ...
  geom_sf(data = constellation_lines_sf_boo, linewidth = 1, color = "#1eff00", size = 3) +
...
# ================================
# Option 1: Colored caption using ggtext
# ================================
caption_colored <- "
<span style='color:white'>DANIELLE PANARIELLO<br></span>
<span style='color:#ff0000'>PSR J0740+6620<br></span>
<span style='color:white'>12th May 1994<br>Queens, NY<br></span>
<span style='color:#1eff00'>Boötes</span>
"

p1 <- p +
  labs(caption = caption_colored) +
  theme(plot.caption = element_markdown(
    hjust = 0.5, face = 2, size = 25,
    margin = margin(150, 20, 20, 20)
  ))

# ================================
# Option 2: Plain white caption (no extra packages)
# ================================
caption_plain <- "DANIELLE PANARIELLO\nPSR J0740+6620\n12th May 1994\nQueens, NY\nBoötes"

p2 <- p +
  labs(caption = caption_plain) +
  theme(plot.caption = element_text(
    color = "white", hjust = 0.5, face = 2, size = 25,
    margin = margin(150, 20, 20, 20)
  ))

# To render either plot, just call:
# print(p1)  # colored caption
# print(p2)  # plain white caption

########################################
# # Plot without caption
# ggsave('./figs/queens_new_york_time_with_star_and_constellation_Boötes.png', plot = p, width = unit(10, 'in'), 
#        height = unit(15, 'in'))

# # Plot with colored caption
ggsave('./figs/queens_new_york_time_with_star_and_constellation_Boötes_colored_text.png', plot = p1, width = unit(10, 'in'), 
       height = unit(15, 'in'))

# # Plot with white caption
ggsave('./figs/queens_new_york_time_with_star_and_constellation_Boötes_white_text.png', plot = p2, width = unit(10, 'in'), 
       height = unit(15, 'in'))
