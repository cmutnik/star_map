...
honolulu_hi_location <- "+proj=laea +x_0=0 +y_0=0 +lon_0=21.315603 +lat_0=22.141907" # 21.315603, -157.858093 but `lat < 90` for laea

########################################
flip <- matrix(c(-1, 0, 0, 1), 2, 2)
########################################
hemisphere <- st_sfc(st_point(c(21.315603, 22.141907)), crs = 4326) |>
              st_buffer(dist = 1e7) |>
              st_transform(crs = honolulu_hi_location)
########################################
...
st_crs(constellation_lines_sf) <- honolulu_hi_location
########################################
...
st_crs(stars_sf) <- honolulu_hi_location
########################################
library(grid)
...
url_other_constellations <- "https://raw.githubusercontent.com/cmutnik/star_map/ariyeh/data/aqr.constellations.lines.json"
constellation_lines_sf_other <- st_read(url_other_constellations, stringsAsFactors = FALSE) %>%
  ...
  filter(!is.na(st_is_valid(.))) %>%
  mutate(geometry = geometry * flip) 
st_crs(constellation_lines_sf_other) <- honolulu_hi_location
########################################
# Color specific constellations
url_aqr_constellation <- "https://raw.githubusercontent.com/cmutnik/star_map/ariyeh/data/aqr.constellations.lines.json"
constellation_lines_sf_aqr <- st_read(url_aqr_constellation, stringsAsFactors = FALSE) %>%
  st_wrap_dateline(options = c("WRAPDATELINE=YES", "DATELINEOFFSET=90")) %>% 
  st_transform(crs = honolulu_hi_location) %>%
  st_intersection(hemisphere) %>%
  filter(!is.na(st_is_valid(.))) %>%
  mutate(geometry = geometry * flip) 
st_crs(constellation_lines_sf_aqr) <- honolulu_hi_location
########################################
...
#   st_intersection(hemisphere) %>%
#   mutate(geometry = geometry * flip) 
# st_crs(stars_sf_ns_j0740) <- honolulu_hi_location
########################################
########################################
########################################
p <- ggplot() +
  ...
  geom_sf(data = constellation_lines_sf, linewidth = 1, color = "#ffffff",
          size = 2) +
  geom_sf(data = constellation_lines_sf_aqr, linewidth = 1, color = "#ff3700e4",
          size = 3) +
...
########################################
ggsave('./figs/ariyeh_honolulu.png', plot = p, width = unit(10, 'in'),
       height = unit(15, 'in'))
