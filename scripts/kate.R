...
greenwich_kates_birth_location <- "+proj=laea +x_0=0 +y_0=0 +lon_0=41.027054 +lat_0=-73.620071"

########################################
flip <- matrix(c(-1, 0, 0, 1), 2, 2)
########################################
hemisphere <- st_sfc(st_point(c(41.027054, -73.620071)), crs = 4326) |>
              st_buffer(dist = 1e7) |>
              st_transform(crs = greenwich_kates_birth_location)
########################################
...
st_crs(constellation_lines_sf) <- greenwich_kates_birth_location
########################################
...
st_crs(stars_sf) <- greenwich_kates_birth_location
########################################
library(grid)
...
url_other_constellations <- "https://raw.githubusercontent.com/cmutnik/star_map/kate/data/cap.constellations.lines.json"
constellation_lines_sf_other <- st_read(url_other_constellations, stringsAsFactors = FALSE) %>%
  ...
  filter(!is.na(st_is_valid(.))) %>%
  mutate(geometry = geometry * flip) 
st_crs(constellation_lines_sf_other) <- greenwich_kates_birth_location
########################################
# # Color specific constellations
url_cap_constellation <- "https://raw.githubusercontent.com/cmutnik/star_map/kate/data/cap.constellations.lines.json"
constellation_lines_sf_cap <- st_read(url_cap_constellation, stringsAsFactors = FALSE) %>%
  st_wrap_dateline(options = c("WRAPDATELINE=YES", "DATELINEOFFSET=90")) %>% 
  st_transform(crs = greenwich_kates_birth_location) %>%
  st_intersection(hemisphere) %>%
  filter(!is.na(st_is_valid(.))) %>%
  mutate(geometry = geometry * flip) 
st_crs(constellation_lines_sf_cap) <- greenwich_kates_birth_location
########################################
...
#   st_intersection(hemisphere) %>%
#   mutate(geometry = geometry * flip) 
# st_crs(stars_sf_ns_j0740) <- greenwich_kates_birth_location
########################################
########################################
########################################
p <- ggplot() +
  ...
  geom_sf(data = constellation_lines_sf, linewidth = 1, color = "#ffffff",
          size = 2) +
#   geom_sf(data = constellation_lines_sf_cap, linewidth = 1, color = "#ff3700e4",
#           size = 3) +
...
########################################
ggsave('./figs/kates_birth_location_greenwich.png', plot = p, width = unit(10, 'in'),
       height = unit(15, 'in'))
